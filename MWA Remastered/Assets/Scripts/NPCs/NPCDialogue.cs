...
public class NPCDialogue : MonoBehaviour, ILocalization
{
    [HideInInspector]
    ...
    public bool autoLocalize, localizeOnAwake = false;
    ...
    public UnityEvent onUpdate, onLocalize;

    public void Awake()
    {
        if (controlledFromOutside && onAwake != null) onAwake.Invoke();
        if (localizeOnAwake) AmongLocalize();
    ...
    {
        if (!autoLocalize || localizeOnAwake) return;
        AmongLocalize();
    }

    public void ManualLocalize()
    {
        for (int i = 0; i < currentlySayingDialogs.Length; i++)
        {
            currentlySayingDialogs[i] = LocalManager.Localize(currentlySayingDialogs[i]);
        }
    }

    public void AmongLocalize()
    {
        nameToDisplay = LocalManager.Localize(nameToDisplay);
        option1 = LocalManager.Localize(option1);
        option2 = LocalManager.Localize(option2);
        dialogs = LocalManager.LocalizeArray(dialogs);
        dialogsOption1 = LocalManager.LocalizeArray(dialogsOption1);
        dialogsOption2 = LocalManager.LocalizeArray(dialogsOption2);
