...
public class KnifeGiver : NPCDialogue, IUseSaveGame
{
    public string[] afterKnife;
    public GameObject knifePrefab;
    public Vector3 knifePos;
    public bool knifeGiven;

    public void SussyLocalize()
    {
        if (!autoLocalize) return;
        afterKnife = LocalManager.LocalizeArray(afterKnife);
        if (knifeGiven) currentlySayingDialogs = afterKnife;
    }
    ...
    {
        knifeGiven = SaveGame.knifegiverGaveKnife;
        if (knifeGiven)
        {
            canOption = false; optionChoice = false;
            currentlySayingDialogs = afterKnife;
